...
It rewards *process*, not *result*. Someone who turns up every day and finishes their block
...
| --- | --- | --- |
| `DATABASE_URL` | *(unset)* | Postgres connection string. When unset, the app uses embedded PGlite. Required in production. |
...
- **Integration** (against real Postgres) — ledger idempotency under sequential *and*
...
The interface is mobile-first — the students using it are on phones between postings. The
visual identity is original: an ink-navy base for medical credibility, a mint "pulse" accent
that carries every action, and a flame gradient used **only** for streaks, so the flame
always means the same thing.

Motion is used to confirm behaviour, never to decorate. Anything a number depends on is
driven by authoritative CSS rather than an animation, so a throttled tab or an interrupted
transition can never leave a progress bar showing a figure that isn't true. Reduced motion is
respected throughout, and status is never communicated by colour alone.
