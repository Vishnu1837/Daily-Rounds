...
export type AdminStudentRow = {
  ...
  /** Today's show-up rests on a hand mark alone. See `showedUpOnMarkAlone`. */
  handMarkedToday: boolean;
...
export const getCohortStudents = cache(async function getCohortStudents(
  ...
  return members.map((m) => {
    ...
    // Reporting only. See `showedUpOnMarkAlone`.
    const handMarkedOnly = (d: ISODate) => days.get(d)?.handMarkedOnly ?? false;
    ...
    return {
      ...
      handMarkedToday: handMarkedOnly(today),
...
export type CohortOverview = {
  size: number;
  /** Students who turned up today, by any route. This is the turnout number. */
  activeToday: number;
  attendanceToday: number;
  attendanceMarked: number;
  /**
   * How many of `activeToday` rest on a cohort lead's mark and nothing else.
   *
   * Not deducted from anything — it is the split behind the headline, so a bulk mark is
   * legible as a bulk mark rather than hidden inside a single percentage. See
   * `showedUpOnMarkAlone`.
   */
  handMarkedToday: number;
...
export async function getCohortOverview(ctx: CohortCtx): Promise<CohortOverview> {
  ...
  const [turnoutRows, students] = await Promise.all([
    db
      .select({
        ...
        /* Reporting only — see `showedUpOnMarkAlone`. Never subtracted from anything. */
        handMarked: sql<number>`count(*) FILTER (WHERE ${dailyActivity.handMarkedOnly})::int`,
  ...
  return {
    ...
    handMarkedToday: active.filter((s) => s.handMarkedToday).length,
...
export async function getCohortMaterials(cohortId: string) {
  return db
    .select({
      ...
      // Never sent to the browser as-is; the page turns it into a cache-busting version.
