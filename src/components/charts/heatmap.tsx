 ...
 * drawn as an outline, which is what makes a gap in the grid legible without being punitive
 * about weekends.
 *
 * `bonus` is the one band outside the indigo scale, and deliberately so: it is a rest day
 * the student worked anyway, which is not a *better* scored day but a different kind of day.
 * It takes citrus, the ramp this system reserves for earned XP, and an outline that says it
 * sits outside the required grid rather than higher up it.
 */
export const BAND_STYLE: Record<DayBand, { className: string; label: string }> = {
  ...
  bonus: {
    className: 'bg-citrus-400/30 ring-1 ring-inset ring-citrus-500/55',
    label: 'Bonus day',
  },
};
